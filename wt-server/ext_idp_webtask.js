...
let lastLogin = null;
const JWKS_URI = "https://icjoseph.eu.auth0.com/.well-known/jwks.json";
const AUDIENCE = "https://icjoseph.eu.auth0.com/api/v2/";
const ISSUER = "https://icjoseph.eu.auth0.com/";
...
app.post("/", jsonParser, function(req, res, next) {
  ...
  if (!reqBody || !reqBody["api_url"] || !reqBody["user_id"]) {
    return res.status(400).json({
      error: "api_url or user_id missing"
    });
  }
...
function getAccessToken(context, decoded, cb) {
  if (
    !accessToken ||
    !lastLogin ||
    moment(new Date()).diff(lastLogin, "minutes") > 30
  ) {
    const options = {
